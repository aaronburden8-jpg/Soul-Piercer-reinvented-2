...
const Tooltip: React.FC<{ text: string; children: React.ReactNode; duration?: number }> = ({ text, children, duration = 300 }) => {
  ...
  return (
    <div className="relative inline-block" onMouseEnter={onEnter} onMouseLeave={onLeave}>
      {children}
      {show && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-64 p-4 glass-panel rounded-2xl text-[10px] text-white font-mono uppercase tracking-[0.2em] text-center z-[100] pointer-events-none animate-tooltip-in">
          {text}
        </div>
      )}
    </div>
...
const App: React.FC = () => {
  ...
  const handleGenerate = async (seriesContext?: ActiveSeries) => {
    ...
    try {
      ...
      if (errStr.includes('429') || errStr.includes('quota') || errStr.includes('resource_exhausted')) {
        ...
        msg = "The Archives are recalibrating. Connection has been refreshed.";
      }
  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-16 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row items-center justify-between mb-12 glass-panel p-10 rounded-[3.5rem] border border-white/10 shadow-2xl">
      ...
      <main>
        {error && (
          <div className="mb-12 bg-red-500/10 border border-red-500/20 p-10 rounded-[2.5rem] text-red-100 font-mono text-center shadow-2xl animate-shake">
        ...
        {!devotional ? (
          <div className="space-y-12 animate-slide-up">
            ...
            <div className="flex flex-col items-center gap-6">
              ...
              {mode === 'journey' && (
                <div className="animate-slide-up flex items-center gap-6 glass-panel px-10 py-5 rounded-2xl border border-emerald-500/20">
            ...
            <div className="relative">
              ...
              <button onClick={() => handleGenerate()} disabled={loading} className="absolute bottom-12 right-12 px-16 py-6 rounded-2xl bg-emerald-500 text-white font-mono font-black uppercase tracking-widest shadow-2xl transition-all hover:scale-105 disabled:opacity-20 flex items-center gap-4">
                {loading ? <Icons.Loader className="w-6 h-6" /> : <Icons.Send className="w-6 h-6" />}
