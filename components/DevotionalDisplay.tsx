...
const DevotionalDisplay: React.FC<Props> = ({ devotional }) => {
  ...
  const parseSections = (markdown: string): DevotionalSection[] => {
    ...
    // The very first part might be text BEFORE any header
    const firstPart = parts[0].trim();
    if (firstPart) {
      sections.push({ title: "Analysis Summary", content: firstPart });
    }
  ...
  const startDeepDive = async () => {
    ...
    setIsDiving(true);
    setDiveContent("");
    setDiveStatus("Initializing Research Link...");
    ...
    try {
      ...
      setDiveStatus(`Connection Failed: ${err.message || 'Unknown fault'}`);
